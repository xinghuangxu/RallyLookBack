...
class RallyLookBack {
    ...
    public function __construct($username, $password, $domain = 'rally1.rallydev.com') {
        ...
        $workspace = Rally::getInstance()->get('workspace',"","true");
        print_r($workspace);
    ...
    public function findWithObjectId($objectId){
        return $this->update('userstory',"",array("find"=>array(
            "ObjectId"=>$objectId
        )));
    }
...
//        }
    ...
    protected function _execute($method) {
        $method = ltrim($method, '/');
        $url = "https://{$this->_domain}/analytics/{$this->_version}/service/rally/workspace/$this->_workspace/artifact/snapshot/query.js";
        print_r($url);
